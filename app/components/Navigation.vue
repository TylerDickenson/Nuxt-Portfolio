<template>
  <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 flex justify-center px-4"
    ...
    <div class="border-2 border-gray-400 rounded-full px-4 py-0 mt-4 bg-white dark:bg-gray-800 dark:text-gray-300 shadow flex items-center justify-center w-fit">
        ...
        <template v-for="(item, idx) in navItems" :key="item.to">
          <li>
            <a
              ...
              class="font-centurion text-lg inline-block px-1 py-2 rounded-full transition-all duration-300 transform hover:scale-105 text-gray-800 hover:text-black dark:text-gray-300 dark:hover:text-white"
            >
              {{ item.label }}
            </a>
          </li>
          <div
            ...
            class="h-5 w-px bg-gray-300 dark:bg-gray-600 self-center"
          ></div>
        </template>
      </ul>
    </div>
